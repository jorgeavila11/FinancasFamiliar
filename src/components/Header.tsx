...
const Header: React.FC<HeaderProps> = ({ profile, onNavigate }) => {
  ...
  return (
    <header className="bg-surface/80 backdrop-blur-md border-b border-border-card sticky top-0 z-[100] h-16 flex items-center justify-between px-6 shadow-[0_1px_3px_0_rgba(0,0,0,0.02)]">
      <div className="flex items-center gap-4 relative" ref={menuRef}>
        <div 
          className={cn(
            "w-10 h-10 rounded-full border-2 overflow-hidden cursor-pointer transition-all duration-300",
            isMenuOpen ? "border-primary scale-110 shadow-lg shadow-primary/20" : "border-slate-200 dark:border-slate-700 hover:border-primary/50"
          )}
        ...
        <AnimatePresence>
          {isMenuOpen && (
            <motion.div 
              ...
              className="absolute top-14 left-0 w-64 bg-surface rounded-[1.5rem] shadow-2xl border border-border-card p-2 overflow-hidden z-[101]"
            >
              <div className="px-4 py-3 border-b border-surface-variant mb-2">
                <p className="font-bold text-on-surface truncate">{profile?.displayName || 'Usuário'}</p>
                <p className="text-[10px] text-on-surface-variant font-bold uppercase tracking-widest truncate">{profile?.email}</p>
              </div>
              
              <div className="space-y-1">
                <button 
                  onClick={() => handleNavigate('settings')}
                  className="w-full flex items-center gap-3 px-4 py-2 text-sm font-semibold text-on-surface-variant hover:bg-surface-variant rounded-xl transition-colors"
                >
        ...
        <h1 className="font-manrope font-extrabold text-on-surface tracking-tight text-lg">Minhas Finanças</h1>
      </div>
